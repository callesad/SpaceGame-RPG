...
public class LazerBehaviour : MonoBehaviour
{
    ...
    private Vector3 positionDiff; //distance from source
    ...
    void Update()
    {
        positionDiff = transform.position - source.transform.position;
    ...
    void OnTriggerEnter2D(Collider2D target)
    {
        ...
        if (!target.CompareTag("Trigger")) {Destroy(gameObject);} //despawn lazer

//--------debugging------------------------------------------------------------------//
        if (!target.CompareTag("Trigger")) Debug.Log("lazer hit " + target);         //
//-----------------------------------------------------------------------------------//
